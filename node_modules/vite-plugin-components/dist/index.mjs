...
async function generateDeclaration(ctx, root, filepath) {
  const lines = Object.values(ctx.componentNameMap).map(({path, name, importName}) => {
    const related = slash(path).startsWith("/") ? `./${relative2(dirname2(filepath), resolve2(root, path.slice(1)))}` : path;
  ...
  return {
    ...
    configResolved(config) {
      ...
      if (options.globalComponentsDeclaration) {
        ctx.searchGlob();
        const path = resolve3(config.root, typeof options.globalComponentsDeclaration === "string" ? options.globalComponentsDeclaration : "components.d.ts");

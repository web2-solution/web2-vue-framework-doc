...
var import_windicss = __toModule(require("windicss"));
...
async function resolveOptions(options = {}, utilsOptions = {}, loadConfigFile = false) {
  ...
  let {resolved: config, configFilePath} = loadConfigFile ? await loadConfiguration(__spreadProps(__spreadValues({}, utilsOptions), {
  ...
  if (typeof config === "string" || !config) {
    if (!config) {
      for (const name2 of configureFiles) {
        const tryPath = import_path2.default.resolve(root, name2);
      ...
      configFilePath = import_path2.default.resolve(root, config);
...
function createUtils(userOptions = {}, utilsOptions = {
  ...
  const scan = (0, import_utils6.createSingletonPromise)(async () => {
    ...
    const contents = await Promise.all(files.filter((id) => isDetectTarget(id)).map(async (id) => [await import_fs2.default.promises.readFile(id, "utf-8"), id]));

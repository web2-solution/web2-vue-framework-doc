...
// src/resolveOptions.ts
var jiti = _jiti(__filename, {requireCache: false, cache: false});
...
function buildGlobs(dirs, fileExtensions) {
  ...
  const globs = exts.length ? dirs.map((i) => path.posix.join(i, exts.length > 1 ? `**/*.{${exts.join(",")}}` : `**/*.${exts[0]}`)) : [];
...
function createUtils(userOptions = {}, utilsOptions = {
  ...
  const scan = createSingletonPromise(async () => {
    ...
    const contents = await Promise.all(files.filter((id) => isDetectTarget(id)).map(async (id) => [await fs2.promises.readFile(id, "utf-8"), id]));
